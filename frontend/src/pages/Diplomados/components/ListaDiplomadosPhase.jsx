// src/pages/diplomados/components/ListaDiplomadosPhase.jsx
    ...
    <div className="px-4">
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-200 mb-4">
        <div className="flex justify-between items-center">
          ...
          {/* ✅ Botón Nuevo Diplomado - SOLO en modo admin */}
          {modoAdmin && (
            <button
              onClick={onAbrirFormNuevo}
              className="bg-purple-500 text-white px-3 py-1.5 rounded-lg text-sm flex items-center gap-1 hover:bg-purple-600 transition"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
        {diplomados.map((diplomado) => {
          ...
          return (
            <div 
              key={diplomado.id} 
              className={`bg-white rounded-xl shadow-md border overflow-hidden hover:shadow-lg transition-shadow cursor-pointer ${diplomado.esPremioTorneo ? 'border-amber-400 ring-1 ring-amber-400' : 'border-gray-200'}`}
              onClick={() => onSeleccionarDiplomado(diplomado)}
            >
              {/* Badge de premio */}
              ...
              <div className="h-40 bg-gradient-to-br from-purple-50 to-indigo-100 relative flex items-center justify-center overflow-hidden">
                <img 
                  src={diplomado.imagen || 'https://placehold.co/400x200/purple-100/purple-800?text=Diplomado'} 
                  alt={diplomado.titulo}
                  className="h-full w-full object-cover"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = 'https://placehold.co/400x200/purple-100/purple-800?text=Diplomado';
                  }}
                />
                {progreso > 0 && (
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-purple-600 font-semibold uppercase tracking-wide">
                    {diplomado.modulos?.length || 0} módulos
                  </span>
                  {progreso > 0 && (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 text-amber-500">
                    ...
                    <span className="text-xs font-semibold">{diplomado.duracion || '160 horas'}</span>
                  </div>
                  ...
                  {modoAdmin && (
                    <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button 
                        onClick={() => onEliminar(diplomado.id)}
                      ...
                      <button 
                        onClick={() => onTogglePremio(diplomado.id)}
                        className={`${diplomado.esPremioTorneo ? 'text-amber-500' : 'text-gray-400'} hover:text-amber-500`}
                        title={diplomado.esPremioTorneo ? 'Quitar premio' : 'Marcar como premio'}
                      >
                  ...
                  {/* Botón ver diplomado - SIEMPRE visible */}
                  <button 
                    className="text-purple-600 text-sm font-medium hover:text-purple-800 flex items-center gap-0.5"
                    onClick={(e) => { e.stopPropagation(); onSeleccionarDiplomado(diplomado); }}
                  >
                    Ver diplomado
      ...
      {diplomados.length === 0 && (
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-12 text-center">
          ...
          {modoAdmin && (
            <button onClick={onAbrirFormNuevo} className="mt-3 bg-purple-500 text-white px-4 py-2 rounded-lg">
