...
const MiBilletera = () => {
  const { user } = useAuth();
  const { saldo, transacciones, recargarSaldo, transferir, retirar } = useBilletera();
  ...
  const handleTransferir = () => {
    ...
    setTimeout(() => {
      ...
      if (exito) {
        ...
        alert('❌ Error en la transferencia. El usuario no existe o saldo insuficiente');
      }
      setProcesando(false);
    }, 1500);
  };
  ...
  const handleRetirar = () => {
    ...
    setTimeout(() => {
      ...
      if (exito) {
        alert(`✅ Solicitud de retiro de $${monto} MXN registrada. Se transferirá a tu cuenta en 24-48 horas.`);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Mi Billetera</h1>
      <p className="text-gray-600 mb-6">Gestiona tu saldo, recarga, transfiere o retira dinero</p>

      {/* Tarjeta de saldo */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl shadow-lg p-6 text-white mb-8">
        <p className="text-sm opacity-90">Saldo disponible</p>
        <p className="text-5xl font-bold mt-2">${saldo.toLocaleString()} MXN</p>
        <div className="flex flex-wrap gap-4 mt-6">
          <button onClick={() => setMostrarModalRecarga(true)} className="bg-white text-blue-700 px-6 py-2 rounded-lg font-semibold hover:bg-gray-100">+ Recargar</button>
          <button onClick={() => setMostrarModalTransferir(true)} className="bg-blue-500 border border-white text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-400">Transferir</button>
          <button onClick={() => setMostrarModalRetirar(true)} className="bg-gray-800 text-white px-6 py-2 rounded-lg font-semibold hover:bg-gray-700">Retirar</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-md p-4 text-center">
          <p className="text-gray-500 text-sm">Total recargado</p>
          <p className="text-2xl font-bold text-green-600">${totalRecargado.toLocaleString()} MXN</p>
        </div>
        <div className="bg-white rounded-xl shadow-md p-4 text-center">
          <p className="text-gray-500 text-sm">Total gastado</p>
          <p className="text-2xl font-bold text-red-600">${totalGastado.toLocaleString()} MXN</p>
        </div>
        <div className="bg-white rounded-xl shadow-md p-4 text-center">
          <p className="text-gray-500 text-sm">Transacciones</p>
          <p className="text-2xl font-bold text-blue-600">{transacciones.length}</p>
        </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody>
                {transacciones.slice().reverse().map(t => (
                  <tr key={t.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatearFecha(t.fecha)}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{t.descripcion || t.metodo || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {mostrarModalRecarga && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-center mb-4">Recargar saldo</h2>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleRecarga} disabled={procesando || monto <= 0} className="flex-1 bg-green-500 text-white py-3 rounded-xl font-bold disabled:opacity-50">
                {procesando ? 'Procesando...' : 'Pagar y recargar'}
              </button>
            </div>
          </div>
        </div>
      ...
      {mostrarModalTransferir && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-center mb-4">Transferir saldo</h2>
            ...
            <input type="number" value={monto} onChange={(e) => setMonto(parseInt(e.target.value) || 0)} placeholder="Monto a transferir" className="w-full p-3 border rounded-xl mb-3" />
            <input type="text" value={concepto} onChange={(e) => setConcepto(e.target.value)} placeholder="Concepto (opcional)" className="w-full p-3 border rounded-xl mb-4" />
            <div className="flex gap-3">
              <button onClick={() => setMostrarModalTransferir(false)} className="flex-1 bg-gray-200 py-3 rounded-xl">Cancelar</button>
              <button onClick={handleTransferir} disabled={procesando || monto <= 0 || monto > saldo} className="flex-1 bg-blue-500 text-white py-3 rounded-xl font-bold disabled:opacity-50">
      ...
      {mostrarModalRetirar && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-center mb-4">Retirar saldo</h2>
            <p className="text-center text-gray-600 mb-4">Saldo disponible: ${saldo.toLocaleString()} MXN</p>
            <p className="text-sm text-gray-500 mb-2">Monto a retirar (mínimo $100)</p>
            <input type="number" value={monto} onChange={(e) => setMonto(parseInt(e.target.value) || 0)} placeholder="Monto" className="w-full p-3 border rounded-xl mb-4" />
            <div className="text-sm text-gray-500 mb-4">
              <p>Los retiros se procesan en 24-48 horas a tu cuenta bancaria registrada.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setMostrarModalRetirar(false)} className="flex-1 bg-gray-200 py-3 rounded-xl">Cancelar</button>
              <button onClick={handleRetirar} disabled={procesando || monto < 100 || monto > saldo} className="flex-1 bg-yellow-600 text-white py-3 rounded-xl font-bold disabled:opacity-50">
                {procesando ? 'Procesando...' : 'Solicitar retiro'}
              </button>
            </div>
          </div>
        </div>
