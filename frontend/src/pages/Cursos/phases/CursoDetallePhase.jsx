...
const CursoDetallePhase = ({ curso, onBack, onSeleccionarModulo, estaCompletada, onGenerarConstancia }) => {
  ...
  // Calcular progreso del curso directamente aquí
  const calcularProgreso = () => {
    ...
    curso.modulos.forEach(modulo => {
      ...
      lecciones.forEach(leccion => {
        if (estaCompletada && estaCompletada(curso.id, modulo.id, leccion.id)) {
          completadas++;
        }
      });
    });
    ...
    <div className="px-4 py-3">
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden mb-6">
        <div className="bg-gradient-to-r from-slate-800 to-slate-700 p-6 text-white">
          ...
          <p className="text-gray-300 mt-2">{curso.descripcion}</p>
        ...
        <div className="p-5">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-right">
              <p className="text-sm text-gray-600">Progreso: {progreso}%</p>
          ...
          {(curso.modulos || []).map((modulo) => {
            ...
            const completadasModulo = leccionesLista.filter(lec => estaCompletada && estaCompletada(curso.id, modulo.id, lec.id)).length;
          ...
          {estaCompleto && (
            <div className="mt-6">
              ...
              <p className="text-center text-sm text-green-600 mt-3">✅ ¡Felicidades! Has completado este curso al 100%</p>
            </div>
          )}
        </div>
      </div>
