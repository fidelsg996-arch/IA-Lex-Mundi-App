    ...
    <div className="px-4">
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-200 mb-4">
        <div className="flex justify-between items-center">
          ...
          {modoAdmin && (
            <button onClick={onAbrirFormNuevo} className="bg-blue-500 text-white px-3 py-1.5 rounded-lg text-sm flex items-center gap-1 hover:bg-blue-600 transition">
            ...
            >
              ...
              <div className="h-40 bg-gradient-to-br from-blue-50 to-indigo-100 relative flex items-center justify-center overflow-hidden">
                <img 
                  ...
                  onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/400x200/blue-100/blue-800?text=Curso'; }} 
                />
                {/* Botones de admin en hover - SOLO VISIBLES AL HACER HOVER */}
                {modoAdmin && hoverCurso === curso.id && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center gap-3">
                    <button 
                      onClick={(e) => { e.stopPropagation(); onEditar(curso); }} 
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); onEliminar(curso.id); }} 
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); onTogglePremio(curso.id); }} 
                      className={`${curso.esPremioTorneo ? 'bg-amber-500' : 'bg-gray-500'} text-white p-2 rounded-full hover:bg-amber-600 transition`}
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-blue-600 font-semibold uppercase tracking-wide">{curso.modulos?.length || 0} módulos</span>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 text-amber-500"><span className="material-symbols-outlined text-sm">star</span><span className="text-xs font-semibold">{curso.nivel || 'Intermedio'}</span></div>
                  {/* Botones de admin fuera del hover (versión compacta) */}
                  {modoAdmin && (
                    <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                      <button onClick={() => onEditar(curso)} className="text-blue-500 hover:text-blue-700" title="Editar"><span className="material-symbols-outlined text-sm">edit</span></button>
                      <button onClick={() => onEliminar(curso.id)} className="text-red-500 hover:text-red-700" title="Eliminar"><span className="material-symbols-outlined text-sm">delete</span></button>
                      <button onClick={() => onTogglePremio(curso.id)} className={`${curso.esPremioTorneo ? 'text-amber-500' : 'text-gray-400'} hover:text-amber-500`} title={curso.esPremioTorneo ? 'Quitar premio' : 'Marcar como premio'}><span className="material-symbols-outlined text-sm">emoji_events</span></button>
                    </div>
                  )}
                  <button className="text-blue-600 text-sm font-medium hover:text-blue-800 flex items-center gap-0.5" onClick={(e) => { e.stopPropagation(); onSeleccionarCurso(curso); }}>Ver curso<span className="material-symbols-outlined text-xs">arrow_forward</span></button>
                </div>
              </div>
            </div>
          );
      ...
      {cursos.length === 0 && (
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-12 text-center">
          ...
          {modoAdmin && <button onClick={onAbrirFormNuevo} className="mt-3 bg-blue-500 text-white px-4 py-2 rounded-lg">Crear primer curso</button>}
        </div>
      )}
